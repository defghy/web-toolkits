...
```bash
$ npm install @yuhufe/wtool-pixel-core
```
...
import { PixelPalette, usePixFunc, usePixEvent } from '@yuhufe/wtool-pixel-core'

export default defineComponent({
  components: { PixelPalette },
  setup() {
    const pixelRef = ref()
    const { centerAndPosition } = usePixFunc({ comp: pixelRef })
    const { when } = usePixEvent({ comp: pixelRef })

    when({
      onGridHover({ r, c, grid, layerId }) {
        console.log(r, c, grid, layerId)
      },
    })

    return { pixelRef, centerAndPosition }
  },
})
<script>
```
...
|-----------------------------|------------------------------------------------|--------------------------------------------------------|
| [pixelData](#api-pixelData) | 像素图数据                                     | required                                               |
...
<section id="api-grid">
  ...
  import { KonvaComps } from '@yuhufe/wtool-pixel-core'
  ...
  export default defineComponent({
    ...
    props: {
      grid: { type: Object as PropType<any>, required: true }, // 当前渲染的格子
      gridSize: { type: Number, required: true }, // 格子尺寸
...
```typescript
const pixelRef = ref()
const { getStage, scaleByCenter, positionByDelta, centerAndPosition } = usePixFunc({ comp: pixelRef })
```

| Name              | Description                                                   | Type                                                                                                  |
|-------------------|---------------------------------------------------------------|-------------------------------------------------------------------------------------------------------|
| getStage          | 获取`konva`的`stage`对象                                      | [Konva.Stage](https://konvajs.org/docs/overview.html)                                                 |
| stage             | 内部缓存的 stage ref                                          | `Ref<Konva.Stage>`                                                                                    |
| getHistoryDo      | 仅在`useUndo`时返回有效值                                     | `() => { undo, redo, undoStatus, redoStatus }`                                                        |
| centerAndPosition | 棋盘移动到容器中心，宽高尽量充满容器                           | `() => Promise<void>`                                                                                 |
...
when({
  onGridHover({ r, c, grid, layerId }) {
    state.currGrid = { r, c, groupId: grid.groupId, layerId }
  },
})
```
...
|---------------------------------------|--------------------------------------------|-------------------------------------------------|
| onGridHover                           | 指针经过格子时触发，一个grid触发一次        | `{ r, c, grid, layerId }`                       |
...
<section id="event-onGridPressed">
  ...
  type GridPressedParams = {
    ...
    passByGrids: { r: number; c: number; grid: PixelGridData }[]; // 本次按下后，已经过的格子数组
  };
  ```
</section>
